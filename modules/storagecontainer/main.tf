locals {
  sanitized_sa_name = replace(replace(lower("stdmu${var.sa_config.name}${var.environment}"), "[^a-z0-9]", ""), "-", "")
}
...
resource "azurerm_private_endpoint" "saendpoint" {
  ...
  name                = "pe-${azurerm_storage_account.sa.name}-${var.environment}"
  location            = data.azurerm_resource_group.existing.location
  resource_group_name = data.azurerm_resource_group.existing.name
  ...
  private_service_connection {
    name                           = "psc-${azurerm_storage_account.sa.name}-${var.environment}"
  ...
  ip_configuration {
    name               = "ip-config-${azurerm_storage_account.sa.name}-${var.environment}-${each.value.type}"
